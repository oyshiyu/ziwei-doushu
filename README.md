...
第一次打开命盘时，建议先按下面顺序看，不必一开始就读完所有宫位：

1. **确认中宫信息**：先核对公历、农历、年干支、时辰、命宫、身宫和五行局是否符合输入。
2. **先看命宫**：命宫是基本格局的起点，先看命宫主星、性格特质、是否为空宫，以及命宫所在宫位的四化或煞曜提示。
3. **再看三方四正**：点击命宫所在宫位，观察命宫、财帛、官禄、迁移和对宫的连线与星曜组合，用来判断命格如何发用。
4. **最后看重点面板**：再读「命格总览」「本命四化」「格局识别」和右侧 AI 解读。AI 解读是辅助说明，不影响本地排盘结果。

## 验证读盘流程

本地验证时，可以用一组固定输入快速确认首屏读盘体验没有退化：
...
1. 运行 `npm run dev` 后打开 `/chart`，输入上面的出生信息并起盘。
2. 确认页面生成命盘方格、中宫基础信息、命宫/身宫标记和「命格总览」卡片。
3. 点击命宫所在宫位，确认三方四正连线出现，右侧焦点面板切换到该宫位说明。
4. 不配置 AI API Key 时，前 3 步仍应可用；只有调用 AI 解读或追问时才依赖 `/api/interpret` 和对应的模型 Key。
5. 在手机宽度下重复第 1-3 步，确认表单、命盘方格和重点面板仍可滚动阅读。
